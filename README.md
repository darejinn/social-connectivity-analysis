...
발표 「31,122개의 메시지가 드러낸 역설 — 사회적 연결성의 Fermi 진단과 n=1 임상시험」에 사용된 분석 코드입니다. 본인의 카카오톡 대화 로그를 직접 넣어 **재현 가능한 n=1 분석**을 돌려볼 수 있도록 정리했습니다.

> ⚠️ **데이터는 이 레포에 포함되어 있지 않습니다.** 사용자 본인의 카톡 로그를 직접 export 해서 넣어야 합니다.
...
### 5. (선택) Mock 데이터로 먼저 테스트

카톡 파일이 없어도 가상 데이터로 파이프라인이 잘 도는지 확인할 수 있습니다.
...
## 방법론적 한계 (발표 Part 3-G와 연계)

이 코드는 다음 다섯 가지 한계를 그대로 안고 있습니다.

1. **역인과 배제 불가** — 관찰적 데이터
2. **카톡만 관찰** — 대면 대화·식사 미측정
3. **서구 코호트 HR 사용** — 한국 집단주의 보정 없음
4. **Level 분류기 정확도 ~75%** — 규칙 기반 heuristic의 한계 (`02_level_classifier.py` 주석 참조)
5. **3축 가중치 자의성** — 문헌 기반이나 unique 아님

각 분석 함수의 docstring에 해당 한계를 명시했으며, `sensitivity_analysis()` 함수로 민감도 재현이 가능합니다.

---

## 프라이버시

- `data/`, `*.txt`, `*.csv`는 `.gitignore`에 포함되어 있어 실수로 커밋되지 않습니다.
- **절대 본인의 카카오톡 로그를 공개 레포에 올리지 마세요.**
